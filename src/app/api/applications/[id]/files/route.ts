...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Check ownership
    const application = await prisma.jobApplication.findFirst({ where: { id: applicationId, userId: user.id } });
    ...
    return NextResponse.json({ error: "Failed to update job application files" }, { status: 500 });
  }
}
